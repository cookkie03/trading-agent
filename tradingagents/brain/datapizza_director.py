...
The Evaluator is the per-ticker brain (analyze_symbol); the Director fans those
out in parallel (bounded), collects the theses, then applies the portfolio-level
Statute (the aggregate the single ticker can't see).

Parallelism: a bounded thread pool gives real concurrency for the blocking LLM
calls; each worker uses its own DB session.
"""
